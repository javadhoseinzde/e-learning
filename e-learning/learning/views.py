...
from .models import Course, Quiz, StudentProfile, tklif, Payment, TklifAnswer, QuizAnswer
...
class QuizView(ListView):
    template_name = 'learning/QuizView.html'
    context_object_name = "quiz"

    def get_queryset(self):
        courses = self.kwargs.get("courses")
        quiz = Quiz.objects.filter(courses__students=self.request.user)
        return Quiz.objects.filter(courses__students=self.request.user)
...
class UploadQuizAnswer(FormView):
    form_class = QuizAnswerForm
    template_name = "learning/QuizAnswerForm.html"
    success_url =reverse_lazy("learning:home")

    def form_valid(self, form, *args, **kwargs):
        pk = self.kwargs.get("pk")
        new_article = QuizAnswer.objects.create(
            user = User.objects.get(pk=self.request.user.id),
            ansser = form.cleaned_data["ansser"],
        ...
        form.save(commit = False)
        return super(UploadQuizAnswer, self).form_valid(form)
"""
